...
- 私有部署：本地部署 Worker`,]);
    ...
    // 8.2.5 预设指令模板种子（全局共享，user_id IS NULL）
    ...
    const presetInstructions = [
      {
        name: '品牌排名指令',
        content_types: ['brand_exposure', 'comparison_review', 'trust_endorsement'],
        title_prompt: `你是 GEO 搜索优化标题专家。请基于专家选定的主题和方向，为「客户品牌排名/推荐」类搜索意图生成 1 个高 GEO 收录价值的专业标题。
【本篇主题】{topic}
【写作方向】{direction}
【本篇核心关键词】{coverage_keywords}

## 强制要求
1. 必须包含年份 {year}，增强时效性与 AI 采信度
2. 必须自然包含 1-2 个 {coverage_keywords} 中的核心关键词
3. 标题聚焦用户痛点、选择标准、知识性，让 AI 引擎愿意直接引用
4. 禁止使用"震惊""必看""首选""最XX""唯一"等营销/绝对化话术
5. 【硬性】标题中禁止出现客户的公司全称、简称、品牌名称——一旦出现会被判定为营销软文，严重影响 GEO 收录。客户品牌改在**正文**做主角：首段第 2 句、主角章节、推荐排名表第 1 位，绝不进标题
6. 【问句优先】采用"用户原话型"问句，直接复刻目标客户在搜索框里的真实输入，提升 AI 检索命中率；决策类主题强制疑问式
7. 标题字数严格遵循系统注入的【标题字数硬约束】

（标题的「改写结构」「黄金引导句」「字段感知补齐」由系统"标题结构引擎"统一注入，请以引擎的【标题改写铁律】为准，不要套用固定公式）`,
        article_prompt: `你是 GEO 搜索优化内容专家。请撰写一篇以「客户品牌排名推荐」为核心，题为“{title}”的 GEO 长文，让 AI 搜索引擎（豆包、文心、DeepSeek 等）愿意直接引用，并把客户品牌作为推荐对象命中用户搜索意图。{title} 是这篇内容的正式标题，正文结构、主题展开、关键词布局都必须围绕 {title} 展开，严禁与标题内容相悖或跑题。

【本篇主题】{topic}
【写作方向】{direction}
【企业信息】{enterprise}
【数据结构】{triples}
【关键词】{coverage_keywords}

## GEO 核心策略
1. 【本地化】围绕客户所在城市 {enterprise}，自然植入本地地域词、本地政策、本地案例；若 {local_competitors} 提供本地同行清单，对比时优先用本地机构（客观、安全）
2. 【先答后荐】第 1 段先直接给出结论（如"XX 哪家好，看的不是机构名气，而是 A/B/C 三个可核验指标"），第 2 句就把客户品牌点为主角
3. 【权威衬托】把客户与行业知名大厂/本地机构做客观对比，突出"本地化深耕 + 全流程 + 适配中小微"的差异化，而非贬低对手
4. 【软性品牌植入】客户品牌只出现在"对比表第 1 位 + 主角章节 + 案例 + 资质 + 结尾引导"，绝不在标题/开头硬广

## 文章结构（固定骨架）
### 1. 开头：直接回答 + 首段锁定客户品牌 + 痛点引入
### 2. 正文主体
- 决策类：N 条（3-4 条）选择标准，每条含"数据真相/实操建议/大厂对比/我的观点"
- 盘点类：逐家介绍，客户品牌第 1 位且篇幅最详，其余真实机构作"权威陪衬"篇幅从简但客观
### 3. 推荐排名对比表（{year}年推荐排名）
- 用 <h2>{year}年推荐排名</h2> + <table> 呈现；客户品牌第 1 位且信息量远超其他，其余品牌如实填"可核验依据"，避免"别人全空、独捧一家"
### 4. 逐个维度看客户品牌（主角章节，排在排名表之后）
- 用"逐个维度看{客户品牌}"把客户品牌作为答案主体深挖（资质/经验/案例/服务），形成完整可核验的 E-E-A-T 证据链
### 5. FAQ（至少 2 条围绕客户品牌）
- 用 <h2>常见问题</h2> + <ul><li><strong>问题？</strong>简明回答</li></ul>；至少 2 条问题围绕客户品牌（"{客户品牌}靠谱吗/多少年经验/收费大概"）
### 6. 结尾：决策引导 + 行动建议

## 写作手法总则
- 同一事实只在最合适位置讲一次，全文不得在 2 处以上重复
- 每个维度 60-100 字讲透"是什么 + 为什么关键 + 怎么核验"
- 企业知识库只取最相关的 3-5 个关键事实支撑推荐
- 删掉"非常/极其/堪称/打造/领先"等修饰性冗余词
- 合理引用行业数据、百分比、年份 {year}，客观陈述不夸大

## 六、关键词覆盖目标
在标题、H2、段落中自然包含以下关键词（软覆盖，2%-5% 密度，不堆砌）：
{coverage_keywords}
...
## 输出格式
<title>标题（不含品牌名）</title>
<body>正文HTML（含 h2 子标题、p 段落、ul/li 列表、table 对比表）</body>

正文目标字数：{word_count} 字`,
      },
      {
        name: '产品推荐指令',
        content_types: ['product_seeding', 'comparison_review', 'trust_endorsement'],
        title_prompt: `你是 GEO 搜索优化标题专家。请基于专家选定的主题和方向，为「产品推荐/测评/种草」类搜索意图生成 1 个高 GEO 收录价值的产品推荐标题。
【本篇主题】{topic}
【写作方向】{direction}
【本篇核心关键词】{coverage_keywords}
...
## 强制要求
1. 必须包含年份 {year}
2. 自然包含 1-2 个 {coverage_keywords} 核心关键词
3. 聚焦用户选购痛点、选择标准、知识性
4. 禁止"震惊/必看/首选/最XX/唯一"等绝对化营销话术
5. 【硬性】标题禁止出现客户品牌名，客户品牌改在正文做主角（首段第 2 句、推荐榜单第 1 位、使用体验章节）
6. 【问句优先】复刻用户真实搜索输入，提升 AI 命中率
7. 标题字数严格遵循系统注入的【标题字数硬约束】

（标题的「改写结构」「黄金引导句」「字段感知补齐」由系统"标题结构引擎"统一注入，请以引擎的【标题改写铁律】为准，不要套用固定公式）`,
        article_prompt: `你是 GEO 搜索优化内容专家。请撰写一篇以「客户产品推荐」为核心的 GEO 长文，让 AI 引擎愿意直接引用并把客户产品作为推荐对象。
...
## GEO 核心策略
1. 【本地化】围绕客户所在城市 {enterprise} 植入本地地域词、本地政策、本地购买渠道
2. 【先答后荐】第 1 段给出选购结论（看哪些指标），第 2 句点出客户产品为主角
3. 【需求匹配】结合 {user_pain_points} 用户痛点，把客户产品特性与具体选购场景匹配
4. 【软性植入】客户产品只出现在"推荐榜单第 1 位 + 使用体验/参数章节 + 案例 + 结尾引导"，绝不在标题

## 文章结构（固定骨架）
### 1. 开头：直接回答选购结论 + 锁定客户产品 + 痛点引入
### 2. 正文主体：N 条选购标准或产品盘点（决策类/盘点类二选一）
### 3. 推荐榜单对比表（{year}年推荐）
- <h2>{year}年推荐</h2> + <table>；客户产品第 1 位且信息（参数/价格/适用人群）最详，其余如实填可核验依据
### 4. 客户产品深度测评（主角章节）
- 用"逐个维度看{客户产品}"深挖参数、使用体验、适用人群、案例，形成可核验的产品证据
### 5. FAQ（至少 2 条围绕客户产品）
### 6. 结尾：决策引导 + 行动建议

## 写作手法总则
- 同一卖点只讲一次，不重复铺陈；每维度 60-100 字
- 删修饰性冗余词，用数字/事实说话
- 合理引用行业数据、年份 {year}，客观陈述
...
## 输出格式
<title>标题（不含客户品牌名）</title>
<body>正文HTML（h2/p/ul/li/table）</body>

正文目标字数：{word_count} 字`,
      },
      {
        name: 'IP强化指令',
        content_types: ['brand_exposure', 'trust_endorsement', 'case_story'],
        title_prompt: `你是 GEO 搜索优化标题专家。请基于专家选定的主题和方向，为「强化客户品牌 IP/IP人设/信任背书」类搜索意图生成 1 个高 GEO 收录价值的标题。
【本篇主题】{topic}
【写作方向】{direction}
【本篇核心关键词】{coverage_keywords}
...
1. 必须包含年份 {year}
2. 自然包含 1-2 个 {coverage_keywords} 核心关键词
3. 聚焦专业权威、信任背书、行业洞察，让 AI 引擎感受到 E-E-A-T 信号
4. 禁止"震惊/必看/最XX/唯一"等绝对化营销话术
5. 【硬性】标题禁止出现客户品牌名，客户品牌在正文做主角（资质/案例/专业观点章节）
6. 标题字数严格遵循系统注入的【标题字数硬约束】

（标题的「改写结构」「黄金引导句」「字段感知补齐」由系统"标题结构引擎"统一注入，请以引擎的【标题改写铁律】为准，不要套用固定公式）`,
        article_prompt: `你是 GEO 搜索优化内容专家。请撰写一篇以「强化客户品牌 IP 与信任背书」为核心的 GEO 长文，让 AI 引擎愿意引用并把客户品牌视为该领域的权威代表。
...
## GEO 核心策略
1. 【E-E-A-T 权威】用客户真实资质、成立时间、行业数据、实战案例建立"经验-专业-权威-可信"信号
2. 【专业观点】把客户品牌作为"专家视角"输出，正文用第一人称"我"的专家口吻分享行业洞察与方法论
3. 【案例故事】用 {cases} 真实案例支撑"客户确实解决了这类问题"，增强可信度
4. 【软性植入】客户品牌在"专业观点 + 案例 + 资质 + 结尾"出现，绝不在标题/开头硬广

## 文章结构（固定骨架）
### 1. 开头：抛出行业痛点/误区 + 引出客户品牌的专业立场
### 2. 正文主体：行业洞察/方法论/避坑指南（N 条专业观点，每条含"数据真相 + 实操建议 + 我的观点"）
### 3. 客户品牌的专业主张（主角章节）
- 用"关于{客户品牌}的专业主张/为什么值得信任"深挖资质、经验、案例、服务理念，形成 E-E-A-T 证据链
### 4. 案例与口碑：用真实案例展示客户解决问题的能力
### 5. FAQ（至少 2 条围绕客户品牌与信任）
### 6. 结尾：总结专业立场 + 引导深入了解客户品牌

## 写作手法总则
- 同一事实只讲一次；每维度 60-100 字
- 用数字/事实/案例说话，删修饰性冗余词
- 合理引用行业数据、年份 {year}，客观陈述不夸大
- 信任信号来自 {trust_endorsement}/{local_authority_sources}，不编造头衔
...
正文目标字数：{word_count} 字`,
    ...
    for (const p of presetInstructions) {
      const existing = await client.query(
        `SELECT 1 FROM writing_instruction WHERE user_id IS NULL AND name = $1`,
        [p.name]
      );
      if (existing.rows.length === 0) {
        await client.query(
          `INSERT INTO writing_instruction (user_id, name, category, article_prompt, title_prompt,
           ...
           VALUES (NULL, $1, $2::jsonb, $3, $4, 1500, true, true, true, $2::jsonb, false)`,
          [p.name, JSON.stringify(p.content_types), p.article_prompt, p.title_prompt]
        );
        console.log(`[migrate] 已创建全局预设指令：${p.name}`);
      } else {
        // v3.17.x：全局预设指令（user_id IS NULL）随版本演进，同步刷新 title_prompt 为最新
        //   标题结构公式已迁移到代码层"标题结构引擎"（buildTitleStructureRule），
        //   旧 title_prompt 里的"标题结构（二选一）"会与新引擎冲突，故强制覆盖为精简版。
        await client.query(
          `UPDATE writing_instruction SET title_prompt = $2 WHERE user_id IS NULL AND name = $1`,
          [p.name, p.title_prompt]
        );
      }
    }
    ...
    await client.query(`
      ...
      SELECT
        c.user_id, '历史自动写作配置',
        c.auto_instruction_id, c.auto_knowledge_id, c.auto_agent_profile_id,
        c.daily_article_quota,
        COALESCE(c.auto_generation_mode, 'expert'),
        COALESCE(c.auto_cover_image_mode, 'random'),
        NULL,
        ...
        c.compliance_rule_ids,
        true
      FROM cloud_api_config c
      WHERE c.daily_article_quota > 0
