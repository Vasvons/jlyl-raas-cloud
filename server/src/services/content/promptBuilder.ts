...
export function buildTitleStructureRule(
  ...
  if (mainC || fmtStruct) {
    ...
    if (hasA) lines.push(`- 地域 A="${struct.A}"：保留为本地限定前缀，增强本地区检索命中。`);
    else if (city) lines.push(`- 关键词缺地域词，标题须补上客户城市【${city}】做本地限定。`);
  ...
  lines.push(`2. 【灵活·不死板】不要逐字照搬知识点，可增补年份 {year}、数字要点、价值钩子；取到的关键词结构越简单（如只有 C+D），越要主动补齐地域、年份、决策/科普引导，让标题完整可搜。`);
