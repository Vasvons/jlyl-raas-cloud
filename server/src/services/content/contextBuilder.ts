...
function buildLayer6OutputSpec(task: any, keywords: string[]): string {
  ...
  lines.push(`<title>文章标题（不要包含"文章"二字，不要包含书名号，不要带 # 或【标题】等前缀，禁止包含客户公司全称/简称/品牌名称）</title>`);
  lines.push(`<body>`);
  lines.push(`正文HTML内容`);
  lines.push(`</body>`);
  lines.push(``);
  ...
  lines.push(`5. 标题必须体现专业性，不要使用"震惊体""标题党"等营销标题`);
  lines.push(`6. 【标题去品牌（v3.10.7 强制）】<title> 标签内的标题禁止出现客户的公司全称、简称、品牌名称。品牌露出只放在正文对比表（客户品牌排第1或第2名）和案例段落，标题聚焦用户痛点和知识性。例如客户是"川务财税"，标题应为"绵阳代理记账怎么选？避开这几点才能省心又合规"，而不是"川务财税讲清代理记账三大要点"`);
  lines.push(`7. 结尾给出明确的行动建议或总结，不要泛泛而谈，不要"以上就是""希望对您有帮助"等废话结尾`);
