...
async function executeWritingTaskInner(taskId: number, userId: number): Promise<void> {
  ...
  // v2.5.17：彻底改造——不再依赖 AI 插入 <img>，所有图片由代码统一插入
  //   原 bug：通过 prompt 让 AI "请插入 <img>" 不稳定，AI 经常忽略导致文章无配图
  //   修复策略：
  //     1. AI prompt 不再包含图片 URL 和插图指令，AI 只负责写正文
  //     2. 不论 task.cover_image_mode / illustration_count 配置如何，只要图库有图就强制取：
  //        - 1 张封面（除非 cover_image_mode === 'none'）
  //        - 至少 2 张插画（如果配置 >= 2 用配置值，否则默认 2 张；illustration_count === 0 才跳过）
  //     3. AI 生成 contentHtml 后，代码按段落均匀插入所有插画，保证 100% 有图
  ...
  // v2.5.17：illustration_count 默认改为 2（原 0），即使用户没配置也至少插 2 张
  //   - 用户明确配 0 → 尊重"不要图"意愿，不插图
  //   - 用户配 -1 / 未配 / 配 >=1 → 至少取 2 张
  const rawIlluCountConfig = Number(task.illustration_count);
  let illuCountConfig: number;
  if (rawIlluCountConfig === 0) {
    illuCountConfig = 0;  // 明确不要图
  } else if (rawIlluCountConfig > 0) {
    illuCountConfig = Math.min(20, rawIlluCountConfig);  // 用配置值
  } else {
    illuCountConfig = 2;  // -1（自动）或未配 → 默认 2 张
  }
  ...
  for (let i = 0; i < totalCount; i++) {
    try {
      ...
      // v2.4.10：保存当前篇预期插入的插画 URL 列表，用于 AI 未按 prompt 插入时 fallback 自动插入
      let illustrationUrlsForArticle: string[] = [];
        ...
        if (coverMode === 'fixed' && fixedCoverUrl) {
          ...
          try {
            ...
            if (randomCovers.length > 0) {
              ...
              // v2.4.10：random 模式下图库为空时也打印诊断（之前只 auto 模式打印，random 模式静默失败难排查）
              console.warn(`[ArticleGen][图库诊断] 任务 ${taskId} 第 ${i + 1} 篇 ${coverMode} 模式：封面图库查询返回空（userId=${userId}, knowledgeId=${task.knowledge_id}, imageType=cover）。请检查 image_library 表是否有对应记录。`);
            }
          } catch (err) {
            console.warn(`[ArticleGen] 任务 ${taskId} 第 ${i + 1} 篇取随机封面失败:`, err);
          }
        } else if ((coverMode === 'random' || coverMode === 'auto') && !task.knowledge_id) {
          // v2.4.10：补充 knowledge_id 为空的诊断（之前静默跳过）
          console.warn(`[ArticleGen][图库诊断] 任务 ${taskId} 第 ${i + 1} 篇 ${coverMode} 模式：task.knowledge_id 为空，无法取封面图`);
        }
        // v2.5.17：取插画逻辑简化——illuCountConfig 已在外层计算好（0/配置值/默认2）
        //   原 illustration_count === -1（自动）逻辑已合并到外层 illuCountConfig 计算
        if (illuCountConfig > 0 && task.knowledge_id) {
          try {
            // 多取 5 张作为去重缓冲，过滤后取前 illuCountConfig 张
            ...
            if (randomIllus.length > 0) {
              ...
              // v2.5.17：URL 列表保存到外层变量，供代码统一插入使用
              illustrationUrlsForArticle = selectedIllus.map((img: any) => img.url);
            } else {
              console.warn(`[ArticleGen][图库诊断] 任务 ${taskId} 第 ${i + 1} 篇：插画图库查询返回空（userId=${userId}, knowledgeId=${task.knowledge_id}, imageType=illustration）`);
            }
          } catch (err) {
            console.warn(`[ArticleGen] 任务 ${taskId} 第 ${i + 1} 篇取插画失败:`, err);
          }
        }
        // v2.5.17：prompt 不再注入插画 URL 和 <img> 指令
        //   原 prompt: "【插画素材（请在正文合适位置插入以下 <img> 标签）】<img src=...>"
        //   现 prompt: 只告诉 AI "文章配图将由系统自动插入，你只需专注正文内容"
        //   这样 AI 不会浪费 token 处理图片，也不会出现"AI 忽略指令导致没图"的问题
        if (illustrationUrlsForArticle.length > 0) {
          articlePrompt += `\n\n【图片说明】本篇文章配图将由系统自动插入，你不需要在正文中放置任何 <img> 标签，只需保证段落结构清晰（多个 <p> 段落），方便系统按段落均匀配图。`;
        }

        // v2.5.17：诊断日志（保留，便于排查）
        console.log(`[ArticleGen][图库注入] 任务 ${taskId} 第 ${i + 1} 篇: coverUrl=${coverUrlForArticle ? coverUrlForArticle.substring(0, 60) + '...' : '(无)'}, illustrationUrls=${illustrationUrlsForArticle.length}张`);
...
8. 标题必须体现专家视角和专业性，不要营销味重的"爆款""必看""震惊"等词`;
        ...
        // v2.5.17：图片插入完全由代码控制——AI 不再负责插图，代码统一按段落均匀插入
        //   原 v2.4.10/v2.4.11 fallback 逻辑依赖"AI 插入数量不足才补插"，
        //   v2.5.17 改为：直接由代码按段落均匀插入所有 illustrationUrlsForArticle
        //
        //   工作流程：
        //   1. AI 生成 contentHtml（不含任何 <img>，prompt 已明确告知"配图由系统自动插入"）
        //   2. 若 illustrationUrlsForArticle 为空但 task.knowledge_id 存在，重查图库兜底
        //   3. 代码按段落均匀插入所有图片，保证文章 100% 有图
        let effectiveIllusUrls = illustrationUrlsForArticle;
        if (effectiveIllusUrls.length === 0 && task.knowledge_id) {
          // v2.5.17：取图失败时重查兜底（保留原 v2.4.11 逻辑）
          try {
            const retryCount = Math.max(2, Math.min(3, illuCountConfig || 2));
            ...
            if (retryImgs.length > 0) {
              ...
              console.warn(`[ArticleGen][图库Fallback] 任务 ${taskId} 第 ${i + 1} 篇: 原取图为空，重查成功 ${retryImgs.length} 张`);
            } else {
              console.warn(`[ArticleGen][图库Fallback] 任务 ${taskId} 第 ${i + 1} 篇: 重查仍为空（userId=${userId}, knowledgeId=${task.knowledge_id}, imageType=illustration）`);
            }
          } catch (err: any) {
            console.warn(`[ArticleGen][图库Fallback] 任务 ${taskId} 第 ${i + 1} 篇: 重查图库失败:`, err?.message || err);
          }
        }
        if (effectiveIllusUrls.length > 0) {
          // v2.5.17：检测 contentHtml 中是否已存在 <img>（AI 偶尔会自作主张插入）
          //   若已存在 <img>，代码补插缺失数量；若不存在，代码插入全部
          const existingImgMatches = contentHtml.match(/<img[^>]*\ssrc\s*=\s*["'][^"']+["']/gi) || [];
          ...
          if (existingImgCount < effectiveIllusUrls.length) {
            ...
            console.log(`[ArticleGen][代码插图] 任务 ${taskId} 第 ${i + 1} 篇: 现有 ${existingImgCount}/${effectiveIllusUrls.length} 张，代码补插 ${missingUrls.length} 张`);

            // 找到所有 </p> 段落结尾位置，用于按段落均匀插入图片
            const paragraphEnds: number[] = [];
            let searchFrom = 0;
            while (true) {
              const idx = contentHtml.indexOf('</p>', searchFrom);
              if (idx === -1) break;
              paragraphEnds.push(idx + 4);  // 指向 </p> 之后的位置
              searchFrom = idx + 4;
            }
            ...
            if (paragraphEnds.length === 0) {
              ...
              // 按 missingUrls 数量在段落间均匀分布插入位置
              const insertPositions: number[] = [];
              for (let k = 0; k < missingUrls.length; k++) {
                // 在段落总数的 (k+1)/(N+1) 比例位置插入
            ...
            console.log(`[ArticleGen][代码插图] 任务 ${taskId} 第 ${i + 1} 篇: 插入完成，contentHtml 长度=${contentHtml.length}, 总图数=${existingImgCount + missingUrls.length}`);
          } else {
            console.log(`[ArticleGen][代码插图] 任务 ${taskId} 第 ${i + 1} 篇: 已有 ${existingImgCount} 张图，无需补插`);
          }
        }
      }
