...
8. 标题必须体现专家视角和专业性，不要营销味重的"爆款""必看""震惊"等词`;
        ...
        // v2.4.10：AI 插图 fallback —— 如果 AI 未按 prompt 插入 <img> 标签或插入数量不足，
        //           自动按段落均匀插入剩余插画，保证文章正文一定有配图。
        //   场景：AI 经常忽略"请在正文合适位置插入以下 <img> 标签"的指令，
        //         导致 content_html 中无 <img>，用户看到"文章没配图"。
        //   修复：检测 contentHtml 中已存在的 <img> 数量，少于预期时按段落均匀插入剩余图片。
        if (illustrationUrlsForArticle.length > 0) {
          const existingImgMatches = contentHtml.match(/<img\s+[^>]*src=["'][^"']+["']/gi) || [];
          const existingImgCount = existingImgMatches.length;
          if (existingImgCount < illustrationUrlsForArticle.length) {
            const missingUrls = illustrationUrlsForArticle.slice(existingImgCount);
            console.warn(`[ArticleGen][图库Fallback] 任务 ${taskId} 第 ${i + 1} 篇: AI 仅插入 ${existingImgCount}/${illustrationUrlsForArticle.length} 张图，自动补插 ${missingUrls.length} 张`);
