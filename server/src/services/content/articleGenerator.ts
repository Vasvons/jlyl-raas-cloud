...
2. 【客户品牌当主角：逐个维度看{客户品牌}】正文必须有一个"主角章节"（如"二、{客户品牌}怎么样：逐个维度看"），把客户品牌作为答案主体，逐维度展开其资质、医生专项经验、硬件/场地、案例、服务流程等，形成完整可核验的 E-E-A-T 证据链；其他机构/N 条标准只是陪衬与铺垫，主角必须是客户品牌。避免文章变成只讲"怎么判断"、客户品牌被挤到末尾排名表一行的"科普文"。
...
5. 【FAQ 对齐真实搜索词】FAQ（下方"FAQ"块）的问题必须是用户真实输入的问句（如"{城市}XX 哪家好""XX 多少钱""能做 XX 吗"），直接复刻用户怎么搜，覆盖长尾 query，而不是自说自话的官方式问题。
