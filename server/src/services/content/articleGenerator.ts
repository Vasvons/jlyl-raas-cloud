        ...
        if ((illuCount > 0 || autoIlluFallback) && task.knowledge_id) {
          try {
            ...
            if (randomIllus.length > 0) {
              ...
              // v2.4.10：保存 URL 列表到外层变量，供 AI 未按 prompt 插入时 fallback 使用
...
8. 标题必须体现专家视角和专业性，不要营销味重的"爆款""必看""震惊"等词`;
        ...
        // v2.4.10/v2.4.11：AI 插图 fallback —— 如果 AI 未按 prompt 插入 <img> 标签或插入数量不足，
        //   自动按段落均匀插入剩余插画，保证文章正文一定有配图。
        //   场景：AI 经常忽略"请在正文合适位置插入以下 <img> 标签"的指令，
        //         导致 content_html 中无 <img>，用户看到"文章没配图"。
        //   v2.4.11 增强：
        //   1. 正则放宽：兼容 `<img src=...>`（无空格）、单引号、无属性等格式
        //   2. 兜底重查：若 illustrationUrlsForArticle 为空但 task 配置了 illustration_count > 0，
        //      说明之前取图失败，这里再尝试一次（可能是临时网络问题）
        //   3. 即使 task.illustration_count === 0/-1，也检测 content_html 中是否已无 <img>，
        //      若图库实际有图且 content_html 无 <img>，强制插入 1-2 张（防止配置丢失）
        let effectiveIllusUrls = illustrationUrlsForArticle;
        if (effectiveIllusUrls.length === 0 && task.knowledge_id) {
          // v2.4.11：之前取图失败时尝试重新查询
          try {
            const retryCount = Math.max(1, Math.min(3, Number(task.illustration_count) || 1));
        ...
        if (effectiveIllusUrls.length > 0) {
          // v2.4.11：正则放宽，兼容 <img src="...">、<img src='...'>、<img class="x" src="..."> 等格式
          ...
          if (existingImgCount < effectiveIllusUrls.length) {
            ...
            console.warn(`[ArticleGen][图库Fallback] 任务 ${taskId} 第 ${i + 1} 篇: AI 仅插入 ${existingImgCount}/${effectiveIllusUrls.length} 张图，自动补插 ${missingUrls.length} 张`);
            ...
            console.log(`[ArticleGen][图库Fallback] 任务 ${taskId} 第 ${i + 1} 篇: 补插完成，contentHtml 长度=${contentHtml.length}, 总图数=${existingImgCount + missingUrls.length}`);
          }
        }
      }
