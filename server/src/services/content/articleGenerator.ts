...
function isThinkingProcess(text: string): boolean {
  ...
  // 2. 包含思考特征短语（强信号）
  //    v3.19.x：移除 GEO优化/差异化优势——这两个是 GEO 写作的正常业务词，
  //    不是思考特征，误判会把 AI 生成的好标题清空、降级成"关键词原文"标题
  if (/用户(的需求|希望|需要|这次|提供)|我的思考|我需要|我打算|我计划|我考虑|分析一下|思考一下|核心诉求|围绕如何|我(已经|将|会|打算)/i.test(text)) return true;
