...
// v3.17.x：自动写作任务的「重点覆盖城市 + 重点优化主词」候选选项
router.get('/keywordsearchrank/autoWritingFocusOptions', authMiddleware, async (req, res) => {
  try {
    const userId = isAgent(req) ? String(getUserId(req)) : (req.query.userId as string);
    const options = await getAutoWritingFocusOptions(userId);
    res.json({ code: 200, data: options });
  } catch (e: any) {
    console.error('[autoWritingFocusOptions] 查询失败:', e.message);
    res.json({ code: 500, message: '服务器错误: ' + e.message });
  }
});
