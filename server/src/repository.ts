...
export async function getKeywordSearchRank(params: SearchRankParams) {
  ...
  // ============ 真实结果 WHERE 条件 ============
  // 搜索详情只展示命中品牌的记录（brand_matched=true），未命中的不出现
  // 按 tab 类型附加过滤：
  ...
  // - 不传 type: 所有 brand_matched=true 的真实记录
...
/**
 * 获取蒸馏关键词库可用的组合规则屏蔽选项（v2.0.7）
 * 来源：kw_config 表中 config_type='distillate' 的 combos 字段
 * 若用户未保存配置，返回默认组合
 */
...
function detectKeywordCombo(
  keyword: string,
  comboMap: { groups: { name: string; words: string[] }[] }
): string | null {
...
export async function startNewRound(
  ...
  // 前缀词屏蔽 + 组合规则屏蔽：读取任务配置，过滤掉不需要查询的关键词
  // 仅对蒸馏词库（keyword_type=0）生效
  let filteredKeywords = uniqueKeywords;
  const taskConfig = await query(`SELECT exclude_prefixes, exclude_combos, keyword_type, user_id FROM real_collect_task WHERE id = $1`, [taskId]);
  const excludePrefixesJson = taskConfig.rows[0]?.exclude_prefixes;
  const excludeCombosJson = taskConfig.rows[0]?.exclude_combos;
  const taskKeywordType = taskConfig.rows[0]?.keyword_type;
  const taskUserId = taskConfig.rows[0]?.user_id;
  ...
  if (excludePrefixesJson && taskKeywordType === 0) {
    ...
    if (prefixes.length > 0) {
      filteredKeywords = filteredKeywords.filter(kw => !prefixes.some(p => kw.startsWith(p)));
  ...
  // 2. 组合规则屏蔽：反向推导关键词所属组合模式，排除被屏蔽的组合
  if (excludeCombosJson && taskKeywordType === 0) {
    let excludeCombos: string[] = [];
    try {
      excludeCombos = JSON.parse(excludeCombosJson).filter((c: string) => c && c.trim());
    } catch {
      excludeCombos = [];
    }
    if (excludeCombos.length > 0 && taskUserId) {
      ...
      if (comboMap) {
        ...
        filteredKeywords = filteredKeywords.filter(kw => {
          const combo = detectKeywordCombo(kw, comboMap);
          // combo 为 null 表示无法识别，保留（避免误删）
          if (!combo) return true;
          return !excludeCombos.includes(combo);
        });
...
export async function createRealCollectTask(params: {
  ...
  const excludePrefixesJson = (params.keywordType === 0 && params.excludePrefixes && params.excludePrefixes.length > 0)
    ...
    : null;
  // excludeCombos 存储为 JSON 字符串（仅蒸馏词库有效）
  const excludeCombosJson = (params.keywordType === 0 && params.excludeCombos && params.excludeCombos.length > 0)
...
export async function updateRealCollectTask(id: number, params: {
  ...
  if (params.excludeCombos !== undefined) {
    // 仅蒸馏词库（keyword_type=0）保存组合规则屏蔽，品牌词库清空
    const kp = params.keywordType !== undefined ? params.keywordType : 0;
    const filtered = params.excludeCombos.filter(c => c && c.trim());
    const json = (kp === 0 && filtered.length > 0) ? JSON.stringify(filtered) : null;
    sets.push(`exclude_combos = $${paramIdx++}`);
    values.push(json);
  }
 ...
 *
 * v3.8.16：去掉 `AND zt = 1` 条件
 *   原 bug：getCoreKeywordsByUserId 有 zt = 1 过滤，但 getDistillateKeywordsByPage
 *         和 autoGenerateDistillateKeywords 都没有 zt 过滤，导致：
 *         - 用户在关键词管理页面能看到核心关键词（无 zt 过滤）
 *         - 但文章生成时 getCoreKeywordsByUserId 查不到（有 zt = 1 过滤）
 *         - 降级使用 zlgjc 蒸馏词（组合词如"公司注册公司"）
 *   修复：去掉 zt = 1 条件，与 getDistillateKeywordsByPage 保持一致
 *   同时添加诊断日志，记录 userId 和查询结果，方便排查
 */
export async function getCoreKeywordsByUserId(userId: string): Promise<string[]> {
  const result = await query(
    `SELECT id, distillate_keyword, user_id, zt FROM distillate_keyword WHERE user_id = $1 ORDER BY id`,
    [userId]
  );
  const keywords = result.rows.map((r: any) => r.distillate_keyword).filter(Boolean);
  console.log(`[getCoreKeywordsByUserId] userId=${userId}, 命中${result.rows.length}条记录, zt值集合=[${result.rows.map((r:any)=>r.zt).join(',')}], 返回${keywords.length}个关键词: [${keywords.slice(0, 10).join('、')}]`);
  return keywords;
}
...
export async function upsertAeoQuotaConfig(userId: number, data: any): Promise<void> {
  ...
  // v3.12：行业维度合规选用（按行业整组选用，对全平台生效）
  if (data.compliance_industry !== undefined) {
    fields.push(`compliance_industry = $${idx++}`);
    values.push(String(data.compliance_industry || ''));
  }
...
/**
 * v2.2.2: 获取客户最近一份含写作建议的周期报告的 writing_suggestions（原始结构）
 * 用于手动写作任务注入 AEO 写作建议（与自动写作一致的数据源）
 * 返回 { suggestions, period_type, period_end, period_report_id } 或 null
 */
export async function getLatestPeriodReportSuggestions(userId: number): Promise<{
  suggestions: any[];
  period_type: string;
  period_end: string;
  ...
  const result = await query(
    `SELECT id, writing_suggestions, period_type, period_end
     FROM aeo_period_report
     WHERE user_id = $1 AND writing_suggestions IS NOT NULL
     ORDER BY period_end DESC
     LIMIT 1`,
  ...
  let suggestions: any[] = [];
  try {
    suggestions = typeof row.writing_suggestions === 'string'
      ? JSON.parse(row.writing_suggestions)
      : row.writing_suggestions;
  } catch {
    suggestions = [];
  }
  ...
  return {
    suggestions,
    period_type: row.period_type || 'unknown',
    period_end: row.period_end ? String(row.period_end) : '',
    period_report_id: row.id,
  };
}

/**
 * v2.5.29: 获取客户指定类型（daily/weekly/monthly）的最新周期报告及其 writing_suggestions
 * 用于飞轮"创建"按钮基于建议来源类型触发自动写作（非重新生成报告）
 *
 * @returns { reportId, writingSuggestions, periodType, periodEnd } 或 null
 */
export async function getLatestPeriodReportByType(
  userId: number,
  periodType: string
): Promise<{ reportId: number; writingSuggestions: any[]; periodType: string; periodEnd: string } | null> {
  const result = await query(
    `SELECT id, writing_suggestions, period_type, period_end
     FROM aeo_period_report
     WHERE user_id = $1 AND period_type = $2 AND writing_suggestions IS NOT NULL
     ORDER BY period_end DESC
     LIMIT 1`,
    [userId, periodType]
  );
...
export async function getWritingTasks(userId: number, page: number = 1, pageSize: number = 20, autoGenerated?: boolean): Promise<{ list: any[]; total: number }> {
  const offset = (page - 1) * pageSize;
  // v2.1.2：userId <= 0 时不按 user_id 过滤（管理员查看全部客户任务）
  const where: string[] = [];
  const params: any[] = [];
  let idx = 1;
  if (userId > 0) {
    where.push(`t.user_id = $1`);
    params.push(userId);
    idx = 2;
  }
  // v2.0.9 支持 auto_generated 过滤：true=仅自动，false=仅手动，undefined=全部
  if (autoGenerated === true) {
    where.push(`t.auto_generated = true`);
  } else if (autoGenerated === false) {
    where.push(`(t.auto_generated = false OR t.auto_generated IS NULL)`);
  }
  const whereClause = where.length > 0 ? `WHERE ${where.join(' AND ')}` : '';
  const countResult = await query(`SELECT COUNT(*) as total FROM ai_writing_task t ${whereClause}`, params);
  const total = parseInt(countResult.rows[0].total);
  const result = await query(
...
export async function createPublishTask(data: {
  ...
  auto_generated?: boolean; // v2.0.9：飞轮守护进程自动创建标记
  ...
  return withTransaction(async (client: PoolClient) => {
    ...
    const taskResult = await client.query(
      `INSERT INTO publish_task (user_id, article_id, target_platforms, scheduled_at, status, total_count, batch_id, auto_generated)
       ...
       RETURNING id`,
      [data.user_id, data.article_id, data.target_platforms, data.scheduled_at || null, platformsToCreate.length, batchId, data.auto_generated === true ? true : false]
    );
...
export async function getPublishTasks(
  ...
  const offset = (page - 1) * pageSize;
  // v2.1.2：userId <= 0 时不按 user_id 过滤
  const where: string[] = [];
  const params: any[] = [];
  let idx = 1;
  if (userId > 0) {
    where.push(`pt.user_id = $1`);
    params.push(userId);
    idx = 2;
  }
  if (autoGenerated === true) {
    where.push(`pt.auto_generated = true`);
  } else if (autoGenerated === false) {
    where.push(`(pt.auto_generated = false OR pt.auto_generated IS NULL)`);
  }
  const whereClause = where.length > 0 ? `WHERE ${where.join(' AND ')}` : '';
  const totalResult = await query(
    `SELECT COUNT(*) as total FROM publish_task pt ${whereClause}`,
    params
  );
  const result = await query(
    `SELECT pt.*,
...
export async function getPublishTasksGroupedByBatch(
  ...
  const offset = (page - 1) * pageSize;
  // v2.1.2：userId <= 0 时不按 user_id 过滤
  const where: string[] = ['pt.batch_id IS NOT NULL'];
  const params: any[] = [];
  let idx = 1;
  if (userId > 0) {
    where.push(`pt.user_id = $1`);
  ...
  const result = await query(
    `SELECT
       pt.batch_id,
       bool_or(pt.auto_generated) as auto_generated,
 ...
 * 创建飞轮事件日志
 *
 * 由桌面端 flywheelDaemon 在每个关键节点（如轮询、AEO分析完成、自动创建写作任务、
 * 自动创建发布任务、错误等）调用，将事件持久化到云端，供前端展示云端工作日志。
 */
export async function createFlywheelEventLog(params: {
  user_id?: number | null;
  event_type: string; // 如 'tick' / 'aeo_completed' / 'writing_task_created' / 'publish_task_created' / 'error' / 'info'
  message: string;
  data?: any;
}): Promise<number> {
  const result = await query(
    `INSERT INTO flywheel_event_log (user_id, event_type, message, data)
     VALUES ($1, $2, $3, $4)
     RETURNING id`,
    [
      params.user_id ?? null,
...
/**
 * 查询飞轮事件日志（按用户过滤，按时间倒序）
 *
 * 支持按 event_type 过滤；limit 默认 100，上限 500。
 */
export async function getFlywheelEventLogs(params: {
  ...
  const countResult = await query(
    `SELECT COUNT(*) as total FROM flywheel_event_log ${whereClause}`,
    queryParams
  );
  ...
  const result = await query(
    `SELECT id, user_id, event_type, message, data, created_at
     FROM flywheel_event_log
     ${whereClause}
     ORDER BY created_at DESC
 ...
 * 清理旧飞轮事件日志（默认保留 30 天）
 */
export async function cleanOldFlywheelEventLogs(daysToKeep: number = 30): Promise<number> {
  const result = await query(
    `DELETE FROM flywheel_event_log WHERE created_at < NOW() - ($1 || ' days')::INTERVAL`,
    [String(daysToKeep)]
  );
  return result.rowCount || 0;
}
...
export async function getComplianceRules(opts?: {
  ...
  const where = conditions.length > 0 ? `WHERE ${conditions.join(' AND ')}` : '';
  const result = await query(
    `SELECT * FROM platform_compliance_rule ${where} ORDER BY industry ASC, updated_at DESC`,
    params
  );
  return result.rows;
}

/** 获取指定行业的所有合规规则（按行业维度，v3.12 起不再按平台拆分） */
export async function getComplianceRulesByIndustry(industry: string, opts?: {
  onlyActive?: boolean;
  source?: string;
}): Promise<any[]> {
  const conditions: string[] = [`industry = $1`];
  const params: any[] = [industry];
  if (opts?.onlyActive) conditions.push(`is_active = true`);
  if (opts?.source) {
    params.push(opts.source);
    conditions.push(`source = $${params.length}`);
  ...
  const result = await query(
    `SELECT * FROM platform_compliance_rule WHERE ${conditions.join(' AND ')} ORDER BY updated_at DESC`,
    params
  );
  return result.rows;
}
...
/**
 * 获取写作时生效的合规规则（按行业维度，source='manual' + is_active=true）
 * v3.12：行业维度的规则对全平台生效，不再按平台过滤
 */
export async function getActiveManualRulesByIndustry(industry: string): Promise<any[]> {
  const result = await query(
    `SELECT * FROM platform_compliance_rule
     WHERE industry = $1 AND source = 'manual' AND is_active = true
...
/** 获取全部启用中的手动合规规则（未指定行业时使用，对全平台生效） */
export async function getAllActiveManualRules(): Promise<any[]> {
  const result = await query(
    `SELECT * FROM platform_compliance_rule
     WHERE source = 'manual' AND is_active = true
     ORDER BY industry ASC, updated_at DESC`
  );
  return result.rows;
}
 ...
 * v3.12：行业维度，规则对全平台生效，不再按平台过滤
 * @param ids 规则 ID 数组
 * @returns 规则记录数组
 */
export async function getComplianceRulesByIds(ids: number[]): Promise<any[]> {
  if (!ids || ids.length === 0) return [];
  const placeholders = ids.map((_, i) => `$${i + 1}`).join(',');
  const sql = `SELECT * FROM platform_compliance_rule WHERE id IN (${placeholders}) AND source = 'manual' AND is_active = true ORDER BY updated_at DESC`;
  const result = await query(sql, ids);
  return result.rows;
}

/** 创建手动规则（v3.12：按行业维度，platform 固定为 'all'，规则对全平台生效） */
export async function createManualRule(data: {
  ...
  const result = await query(
    `INSERT INTO platform_compliance_rule (platform, rule_title, rule_content, source, industry, updated_at)
     VALUES ('all', $1, $2, 'manual', $3, NOW())
     RETURNING *`,
    [data.rule_title, data.rule_content, data.industry || 'general']
  );
  return result.rows[0];
}
